...
export function Select({ id, value, onChange, options, disabled = false, accent = 'blue' }) {
  ...
  return (
    <select
      ...
      onChange={onChange}
      disabled={disabled}
      className={`w-full px-3.5 py-2 text-sm text-slate-800 bg-white border rounded-lg shadow-2xs focus:outline-none transition-all cursor-pointer disabled:bg-slate-100 disabled:text-slate-400 disabled:cursor-not-allowed ${accentStyle}`}
    >
      {options.map((option) => (
